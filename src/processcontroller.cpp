...
void ProcessController::ConvertToGCode()
{
	string GcodeTxt;

	string GcodeStart = m_GCodeStartText->get_text();
	string GcodeLayer = m_GCodeLayerText->get_text();
	string GcodeEnd = m_GCodeEndText->get_text();
...
void ProcessController::CalcBoundingBoxAndCenter()
{
  ...
  for (uint i = 0 ; i < rfo.Objects.size(); i++) {
    for (uint j = 0; j < rfo.Objects[i].files.size(); j++) {
      Matrix4f M = GetSTLTransformationMatrix (i, j);
...
void ProcessController::DrawGrid()
{
...
void ProcessController::LoadConfig(string filename)
{
	...
	// GCode parameters
	string txt;
	if (not cfg.lookupValue("GCodeStartText", txt))
                txt = "; GCode generated by RepSnapper: http://reprap.org/wiki/RepSnapper_Manual:Introduction\n"
                      "G21                              ;metric is good!\n"
	...
	m_GCodeStartText->set_text (txt);

	if (not cfg.lookupValue("GCodeLayerText", txt))
                txt = "";
	m_GCodeLayerText->set_text (txt);

	if (not cfg.lookupValue("GCodeEndText", txt))
                txt = "G1 X0 Y0 F2000.0       ;feed for start of next move\n"
                      "M104 S0.0              ;Heater off\n";
	m_GCodeEndText->set_text (txt);

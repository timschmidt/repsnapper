#ifndef __VMMLIB_STRING_UTILS__HPP__
#define __VMMLIB_STRING_UTILS__HPP__
...
inline double
toDouble( const std::string& source )
{
...
template < typename T >
inline bool
fromString( const std::string& source, T& target )
{
    std::stringstream ss;
    ss << source;
    ss >> target;
...
template< typename T >
void
split( const std::string& source, T& targets, char splitter = ' ' )
{
    size_t pos = 0;
    size_t startpos;

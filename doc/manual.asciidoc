...
This manual is quite outdated. For compiling and installing please see the README file at https://github.com/timschmidt/repsnapper/blob/master/README after you obtained the sources. Any pre-compiled binaries will be very outdated, too.
...
make
----
Executable (repsnapper) will be left in the top folder. Install with "sudo make install"

==== Fedora ====
===== Installing from yum =====
Repsnapper is currently packaged by jebba for Fedora 14 at this repository: http://repos.fedorapeople.org/repos/jebba/reprap/
----
su
...
Save STL::
    Saves all objects in their current state to a single STL file. 
...
windows::
    double click the repsnapper.exe file within the repsnapper directory

linux::
    type repsnapper/repsnapper into the console window.
